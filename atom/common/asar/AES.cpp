...
// void AES::set_IV(unsigned char * IVCl){
// 	memcpy(iv,IVCl,16);
// 	memcpy(&IVC,iv,8);
// 	memcpy(&IVC,iv+8,8);
// }
...
/******************************************************************************/
std::string AES::getDecryptStr(byte ori[], bool p_pad) {
  int i,j;
  int loops = size/N_BLOCK;
  int outp = N_BLOCK;
  std::string tmp = "";
  printf("%d %d\n", loops, outp);
  for (j = 0; j < loops; j += 1){
  ...
  int index1 = 0, index2 = tmp.length();
  for (int m=0; m<(int)tmp.length(); m++) {
    if (tmp[m] == '{') {
      index1 = m;
      break;
    }
