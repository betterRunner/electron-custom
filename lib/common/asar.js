...
(function () {
  ...
  // XUESHU-WEB
  // 解密文件
  const isAppAsar = (asarPath) => {
    let isAppAsar = false
    if (asarPath.indexOf('app.asar') !== -1) {
      isAppAsar = true
    }
    return isAppAsar
  }
  const isFileNeedDecrypt = (filePath) => {
    let ret = false
    const isEncrypt = filePath.match('(?:.js|.vue)$') && (filePath.search('dist(?:/|\\\\)electron(?:/|\\\\)') === -1)
    if (isEncrypt) {
      ret = true
    }
    return ret
  ...
  exports.wrapFsWithAsar = fs => {
    ...
    fs.readFile = function (pathArgument, options, callback) {
      ...
      fs.read(fd, buffer, 0, info.size, info.offset, error => {
        ...
        // 解密文件
        let buffer2 = buffer
        if (isAppAsar(asarPath) && isFileNeedDecrypt(filePath)) {
          buffer2 = Buffer.from(buffer.map(ele => ele - 1))
        }
        // console.log('filepath', filePath)
        // console.log('asar', asarPath)
        // XUESHU-WEB
        callback(error, encoding ? buffer2.toString(encoding) : buffer2)
      })
    }
    ...
    fs.readFileSync = function (pathArgument, options) {
      ...
      // XUESHU-WEB
      return (encoding) ? buffer2.toString(encoding) : buffer2
    }
